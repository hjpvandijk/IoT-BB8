...
EventGroupHandle_t servo_received_flag;
...
{
    esp_err_t res;
    res = uart_flush_input(s->uart);
    if (res != ESP_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not flush input buffer: %d", res);
    ...
    // ESP_LOGI(SERVO_TAG, "Ignoring %d bytes", bytes_read_ignore);


    int bytes_read = uart_read_bytes(s->uart, rx, rx_size, SERVO_UART_TIMEOUT_ms);
    if (bytes_read < 0)
    {
        ESP_LOGE(SERVO_TAG, "Could not read from UART: %d", bytes_read);
        return SERVO_HAL_ERR;
    }

    // ESP_LOGI(SERVO_TAG, "Read %d bytes from UART, expected %d", bytes_read, rx_size);
...
servo_result_t servo_enable_wheel_mode(servo_handle_t *s)
{
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Failed to unlock SRAM lock for servo=%u: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Failed to set MODE=1 servo=%u: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Failed to lock SRAM for servo=%u: %d", s->servo_id, res);
        return res;
    }
...
servo_result_t servo_init(servo_handle_t *s, uint8_t acc, uint16_t goal_speed)
{
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Failed to set servo acceleration for servo=%u: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Failed to set servo goal speed for servo=%u: %d", s->servo_id, res);
        return res;
    }
...
servo_result_t servo_read_state(servo_handle_t *s)
{
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not read servo %u voltage: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not read servo %u current: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not read servo %u position: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not read servo %u speed: %d", s->servo_id, res);
        return res;
    }
    ...
    if (res != SERVO_OK)
    {
        ESP_LOGE(SERVO_TAG, "Could not read servo %u load: %d", s->servo_id, res);
        return res;
    }
...
{
    xEventGroupSetBits(servo_received_flag, SERVO_DMA_RECEIVED_MSK);
    received_bytes = size;
}

/**
 * Example usage:
 *     servo_init(&servo, 10, 10);
 *     servo_set_speed(&servo, 200);
 *     uart_read_bytes(s->uart, rx, 15, SERVO_UART_TIMEOUT_ms);
 * 
 * while(true) { with some frequency
 *     servo_result_t result = servo_read_state(&servo);
 *     float pos = servo_get_position_degrees(&servo);
 * }
*/